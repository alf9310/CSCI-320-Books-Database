...
class Users(Base):
    ...
    User Constructor
    '''
    def __init__(self, uid, username, password, first_name, last_name):
        self.uid = uid
    ...
    '''
    @classmethod
    def create(cls, session, username, password, first_name, last_name):
        try:
            max_uid = session.query(func.max(cls.uid)).scalar() or 0  # Get the maximum uid, or default to 0 if table is empty
            new_user = cls(uid = max_uid + 1, username=username, password=password, first_name=first_name, last_name=last_name)
